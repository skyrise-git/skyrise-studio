...
export default function GlobalContentPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const snapshot = await get(ref(db, 'content/global'));
        if (snapshot.exists()) {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex items-center gap-3 bg-secondary/5 p-6 rounded-2xl border border-white/5 backdrop-blur-sm shadow-xl">
        ...
        <div>
          <h1 className="text-2xl font-logo uppercase">Global Presence Edit</h1>
          <p className="text-muted-foreground font-code text-sm">Update the global world map section</p>
        </div>
      </div>
